#!/usr/bin/env node
// Reproducible WASM build for tree-sitter-lua.
...
const work = await mkdtemp(path.join(tmpdir(), "grammar-lua-build-"));
await run("git", ["clone", "--no-checkout", "https://github.com/tree-sitter-grammars/tree-sitter-lua.git", "src"], { cwd: work });
...
// Locate produced wasm and copy as lua.wasm.
const fs = await import("node:fs/promises");
const built = (await fs.readdir(buildCwd)).find((f) => f.endsWith(".wasm"));
if (!built) throw new Error("no .wasm produced");
await copyFile(path.join(buildCwd, built), path.join(repoRoot, "lua.wasm"));
const bytes = (await readFile(path.join(repoRoot, "lua.wasm"))).length;
console.log(`lua.wasm: ${bytes} bytes (built from ${pin})`);
