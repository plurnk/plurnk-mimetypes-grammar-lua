#!/usr/bin/env node
// CI byte-identical reproducibility check for lua.wasm.
...
const committed = await readFile(path.join(repoRoot, "lua.wasm"));
const committedHash = createHash("sha256").update(committed).digest("hex");
console.log(`committed lua.wasm sha256: ${committedHash}`);

const work = await mkdtemp(path.join(tmpdir(), "grammar-lua-verify-"));
...
console.log(`rebuilt lua.wasm sha256: ${rebuiltHash}`);
